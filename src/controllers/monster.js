...
class Monster extends Controller {
	...
	async handle(data) {
		return new Promise((resolve) => {
			...
			if (data.form) data.formname = formData[data.pokemon_id] ? this.terminateTelegramMarkdown(formData[data.pokemon_id][data.form]) : ''
					...
					this.getAddress({ lat: data.latitude.toString().substring(0, 8), lon: data.longitude.toString().substring(0, 8) }).then((geoResult) => {
						...
						whocares.forEach((cares) => {
							...
							const view = _.extend(data, {
								...
								addr: this.terminateTelegramMarkdown(this.replaceHtmlEntities(geoResult.addr)),
								...
								city: this.replaceHtmlEntities(geoResult.city),
